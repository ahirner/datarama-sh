...
Ls=$(echo $remote_begin-$remote_end | python -c "import sys; p1, p2 = sys.stdin.read().split('-'); print(' '.join(['-L ' + str(p)+':localhost:'+str(p+$offset_local) for p in range(int(p1), int(p2)) ])) ")
